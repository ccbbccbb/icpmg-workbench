    ...
    <div className="min-h-screen bg-kpmgGray6">
      ...
      <div className="bg-gradient-to-r from-kpmgBlue via-kpmgCobaltBlue to-kpmgPacificBlue px-6 py-8">
        <div className="mx-auto max-w-7xl">
          <Button
            className="mb-7 border-white/30 bg-white/10 text-white hover:bg-white/15"
        ...
        {candidates.map((candidate, index) => (
          <button
            className={cn(
              ...
              selectedCandidateId === candidate.candidateId && "bg-backgroundLight"
            )}
          ...
          >
            ...
            <Image
              ...
              height={44}
              src={`/candidates/${candidate.profileImagePath}`}
              width={44}
            />
      ...
      <div className="rounded-2xl bg-background shadow-sm">
        <div className="grid gap-6 p-6 md:grid-cols-[auto_minmax(0,1fr)_auto]">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {candidate.titleMatches.slice(0, 3).map((title) => (
                <span
                  className="rounded-full bg-kpmgGray6 px-3 py-1 text-kpmgGray2 text-xs"
                  key={title}
                >
                  {title}
                </span>
              ...
              {candidate.prototypePortfolio ? (
                <span className="rounded-full bg-kpmgCobaltBlue/10 px-3 py-1 text-kpmgBlue text-xs">
                  Prototype portfolio
                </span>
              ) : null}
            </div>
          ...
          <div className="flex flex-col items-stretch gap-2 md:w-44">
            <div className="rounded-xl bg-kpmgBlue p-4 text-white">
              <p className="text-white/65 text-xs uppercase tracking-widest">Match score</p>
              <p className="mt-1 font-semibold text-3xl tabular-nums">{candidate.score}</p>
            </div>
            <Button asChild className="rounded-full" variant="outline">
              <a href={`/candidates/${candidate.resumePath}`} rel="noreferrer" target="_blank">
        ...
        {candidate.highlights.map((highlight) => (
          <article className="rounded-xl border border-kpmgGray45/70 p-4" key={highlight.label}>
            <div className="flex items-start gap-2">
              <CheckCircle2 aria-hidden className="mt-0.5 size-4 shrink-0 text-kpmgGreen" />
              <div>
                <p className="font-medium text-foreground text-sm">{highlight.label}</p>
                <p className="mt-1 text-kpmgGray3 text-xs capitalize">{highlight.priority}</p>
              </div>
            </div>
...
function ScoreComparison({
  ...
  const scores = Object.entries(candidate.scoresByJob).sort(([a], [b]) => {
    if (a === currentReqId) return -1;
    if (b === currentReqId) return 1;
